...
switch OutgasModel
    case 'Diffusive'
        ...
        %H2Ot_0 = [H2Ot_0; 2*H2Ot_0(end)-1.005*flipud(H2Ot_0(1:end-1))];
        if abs(H2Ot_0(end) - H2Ot_0(end-1))/abs(H2Ot_0(end) - H2Ot_0(1))>1e-3
        [~,ind] = max(H2Ot_0);
            if ind>4*nx/5
                s = 1;
            else
            ...
            %H2Ot_0 = [H2Ot_0; H2Ot_0(end) + sign(H2Ot_0(end) - H2Ot_0(end-1))*(H2Ot_0(end)-flipud(H2Ot_0(1:end-1)))]; % choose initial water to be nearly symmetric to maintain BC
            H2Ot_0 = [H2Ot_0; H2Ot_0(end) - 1.02*s*(H2Ot_0(end)-flipud(H2Ot_0(1:end-1))) - sign(H2Ot_0(end) - H2Ot_0(end-1))*(H2Ot_0(end)-flipud(H2Ot_0(end-1)))];
            %H2Ot_0 = [H2Ot_0; 2*H2Ot_0(end)-1.0005*flipud(H2Ot_0(1:end-1))];
        else
            H2Ot_0 = [H2Ot_0; flipud(H2Ot_0(1:end-1))];
        end
        H2Ot_0(floor(nx*3/2):end) = H2Ot_0(floor(nx*3/2)); % Force end to be flat to prevent water loss
        x(floor(nx*3/2):end) = x(floor(nx*3/2)) + (x(floor(nx*3/2))-x(floor(nx*3/2)-1))*(1:ceil(nx/2));
        Y_0 = [H2Ot_0; R_0];
end
...
v = 1.1*ViscFun(H2Ot(1:nx),T,Composition);
